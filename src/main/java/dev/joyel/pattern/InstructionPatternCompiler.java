...
public final class InstructionPatternCompiler {
    private static final Set<String> FRAME_TYPES = new HashSet<String>(Arrays.asList(
    ...
    public static Compilation compile(String source, boolean includeMetadata) {
        ...
        List<InstructionPattern.Element> elements = new ArrayList<InstructionPattern.Element>();
        List<PatternDiagnostic> diagnostics = new ArrayList<PatternDiagnostic>();
        ...
        for (int lineIndex = 0; lineIndex < lines.length; lineIndex++) {
            ...
            if (metadata && !includeMetadata) { ignoredMetadata = true; continue; }

            try {
                List<OperandKind> kinds = operandKinds(opcode, instructionClass, tokens);
                List<OperandMatcher> operands = new ArrayList<OperandMatcher>(kinds.size());
                for (int i = 0; i < kinds.size(); i++) {
                    operands.add(compileOperand(tokens.get(i + 1), kinds.get(i)));
                }
                elements.add(new InstructionPattern.InstructionLine(sourceLine, opcode, Collections.unmodifiableList(operands)));
            } catch (IllegalArgumentException e) {
        ...
        if (elements.isEmpty()) {
            ...
            for (InstructionPattern.Element e : elements) {
                if (!(e instanceof InstructionPattern.Gap)) { allGaps = false; break; }
            }
            if (allGaps) diagnostics.add(error(1, 1, "A pattern cannot contain only sequence gaps"));
        }

        boolean valid = true;
        for (PatternDiagnostic d : diagnostics) {
            if (d.severity() == PatternDiagnostic.Severity.ERROR) { valid = false; break; }
        }
        InstructionPattern pattern = valid ? new InstructionPattern(input, includeMetadata, Collections.unmodifiableList(elements)) : null;
    ...
    private static List<OperandKind> operandKinds(String opcode, Class type, List<String> tokens) {
        ...
        if (type == TableSwitchInsnNode.class) {
            ...
            List<OperandKind> kinds = new ArrayList<OperandKind>(count);
            kinds.add(OperandKind.INTEGER); kinds.add(OperandKind.INTEGER);
    ...
    private static OperandMatcher compileOperand(String token, OperandKind kind) {
        ...
        if (kind == OperandKind.INTEGER) return new ExactOperand(kind, decodeInteger(token));
        if (kind == OperandKind.BOOLEAN) return new ExactOperand(kind, decodeBoolean(token));
        if (kind == OperandKind.STRING) return StringOperand.compile(token);
        if (kind == OperandKind.LABEL) return new LabelOperand(decodePatternString(token).literal);
        if (kind == OperandKind.VALUE) return new ExactOperand(kind, AssemblerValueCodec.parse(token));
        if (kind == OperandKind.VALUE_LIST) return new ExactOperand(kind, AssemblerValueCodec.parseList(token));
        if (kind == OperandKind.FRAME_TYPE) {
            ...
            return new ExactOperand(kind, t);
        }
    ...
    private static PatternString decodePatternString(String token) {
        ...
        for (int i = 1; i < token.length() - 1; i++) {
            ...
            if (c == '\\') {
                ...
                if (esc == '*' || esc == '?') {
                    literal.append(esc); regex.append(Pattern.quote(String.valueOf(esc))); continue;
                }
                char decoded;
                ...
                literal.append(decoded); regex.append(Pattern.quote(String.valueOf(decoded)));
            } else if (c == '*') { wildcard = true; regex.append(".*");
            } else if (c == '?') { wildcard = true; regex.append('.');
            } else { literal.append(c); regex.append(Pattern.quote(String.valueOf(c))); }
        }
        regex.append('$');
    ...
    public static final class Compilation {
        ...
        public boolean valid() { return pattern != null; }
        public InstructionPattern pattern() { return pattern; }
        public List<PatternDiagnostic> diagnostics() { return diagnostics; }

        public PatternDiagnostic primaryDiagnostic() {
            for (PatternDiagnostic d : diagnostics) {
                if (d.severity() == PatternDiagnostic.Severity.ERROR) return d;
            }
    ...
    public enum AnyOperand implements OperandMatcher {
        INSTANCE;
        public boolean matches(String candidate, Map<String, String> labels) { return true; }
    }
    ...
    public static final class LabelOperand implements OperandMatcher {
        ...
        public boolean matches(String candidate, Map<String, String> labels) {
            ...
            if (bound == null) { labels.put(name, candidateName); return true; }
    ...
    public static final class StringOperand implements OperandMatcher {
        ...
        private StringOperand(String literal, Pattern glob) { this.literal = literal; this.glob = glob; }
    ...
    public static final class ExactOperand implements OperandMatcher {
        ...
        public ExactOperand(OperandKind kind, Object value) { this.kind = kind; this.value = value; }

        public boolean matches(String candidate, Map<String, String> labels) {
            try {
                Object other;
                if (kind == OperandKind.INTEGER) other = decodeInteger(candidate);
                else if (kind == OperandKind.BOOLEAN) other = decodeBoolean(candidate);
                else if (kind == OperandKind.VALUE) other = AssemblerValueCodec.parse(candidate);
                else if (kind == OperandKind.VALUE_LIST) other = AssemblerValueCodec.parseList(candidate);
                else if (kind == OperandKind.FRAME_TYPE) other = candidate.toUpperCase(Locale.ROOT);
    ...
    private static final class PatternString {
        ...
        final Pattern glob;
        PatternString(String literal, Pattern glob) { this.literal = literal; this.glob = glob; }
    }
}
